...
#Author: autoCreated
my $para_num = "2";
my $template_time = "2015-09-10 08:16:22";
my %para;
@array_pre_flag = ();
@array_appendix_flag = ();
...
$pre_cmd{62517} = "FTPSTATUS=`netstat -antp|grep -i \"listen\"|grep \":21\\>\"|wc -l`;
...
if [ \$FTPSTATUS == 0 ];then  echo \"FTP is not running.FTP check result:true\";elif ([ `ps -ef|grep vsftpd|grep -v \"grep\"|wc -l` != 0 ]);then Check_vsftpd;elif ([ `ps -ef|grep pure-ftpd|grep -v \"grep\"|wc -l` != 0 ]);then Check_pureftpd;else echo \"vsftpd and pure-ftpd are not running,check result:true\";fi;
unset FTPSTATUS;";
$pre_cmd{62903} = "chkconfig --list | grep \"daytime\"";
...
$pre_cmd{62349} = "chkconfig --list | grep \"^printer\"";
$pre_cmd{4482} = "cat \"/etc/login.defs\"";
$pre_cmd{62820} = "chkconfig --list | grep \"^tftp\"";
...
$pre_cmd{63959} = "if [ -s /etc/syslog-ng/syslog-ng.conf ];then a_i=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep \"filter[[:space:]]*f_cron[[:space:]]*{[[:space:]]*facility(cron);[[:space:]]*};\" | wc -l `;fi
if [ -s /etc/syslog-ng/syslog-ng.conf ];then b_i=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep \"destination[[:space:]]*cron[[:space:]]*{[[:space:]]*file(\\\"/var/log/cron\\\")[[:space:]]*;[[:space:]]*};\" | wc -l`;fi
if [ -s /etc/syslog-ng/syslog-ng.conf ];then c_i=`cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\" | grep \"log[[:space:]]*{[[:space:]]*source(src);[[:space:]]*filter(f_cron);[[:space:]]*destination(cron);[[:space:]]*};\" | wc -l`;fi
if [[ \"\$a_i\" -gt 1 && \"\$b_i\" -gt 1 && \"\$c_i\" -gt 1 ]];then echo \"result:true\";else echo \"result:false\";fi";
$pre_cmd{36563} = "ls -al /etc/group";
$pre_cmd{36239} = "ls -al /etc/passwd";
$pre_cmd{32433} = "find / -maxdepth 3 -name hosts.equiv 2>/dev/null";
$pre_cmd{33824} = "cat /etc/shadow | awk '{FS=\":\";ORS=\" \"}{if(\$1~/\\s*[^#]/)if(\$1==\"daemon\"||\$1==\"bin\"||\$1==\"sys\"||\$1==\"adm\"||\$1==\"lp\"||\$1==\"uucp\"||\$1==\"nuucp\"||\$1==\"smmsp\"){if(\$2!=\"!!\")flag++}};END{if(flag==0)print \"ok\";else print \"no\"}'";
...
$pre_cmd{32952} = "if (cat /etc/redhat-release 2>/dev/null  | grep \"Red Hat\");then if (/etc/init.d/vsftpd status 2>/dev/null | grep \"running\");then if [ -s /etc/vsftpd.conf ];then ret=`cat /etc/vsftpd.conf | grep -v \"^[[:space:]]*#\" | grep \"anonymous_enable=NO\"`;elif [ -s /etc/vsftpd/vsftpd.conf ]; then ret=`cat /etc/vsftpd.conf | grep -v \"^[[:space:]]*#\" | grep \"anonymous_enable=NO\"`;fi;else echo \"vsftp is not running,result-true\";fi;fi

if (cat /etc/SuSE-release 2>/dev/null | grep \"SUSE Linux\");then if (/etc/init.d/vsftpd status 2>/dev/null | grep \"running\");then if [ -s /etc/vsftpd.conf ];then ret=`cat /etc/vsftpd.conf | grep -v \"^[[:space:]]*#\" | grep \"anonymous_enable=NO\"`;elif [ -s /etc/vsftpd/vsftpd.conf ]; then ret=`cat /etc/vsftpd.conf | grep -v \"^[[:space:]]*#\" | grep \"anonymous_enable=NO\"`;fi;else echo \"vsftp is not running,result-true\";fi;fi
...
$pre_cmd{33681} = "if (cat /etc/redhat-release 2>/dev/null | grep \"Red Hat\");then if (/etc/init.d/vsftpd status 2>/dev/null | grep \"running\");then if (cat /etc/vsftpd/ftpusers 2>/dev/null | grep \"^[[:space:]]*[^#]\" | grep \"\\broot\\b\");then echo \"vsftpd valid\";else echo \"vsftpd invalid\";fi;else echo \"vsftpd valid\";fi;fi
if (cat /etc/SuSE-release 2>/dev/null | grep \"SUSE\");then if (/etc/init.d/vsftpd status 2>/dev/null | grep \"running\");then if (cat /etc/ftpusers 2>/dev/null | grep \"^[[:space:]]*[^#]\" | grep \"\\broot\\b\");then echo \"vsftpd invalid\";else echo \"vsftpd valid\";fi;else echo \"vsftpd is not running; vsftpd valid\";fi;fi";
$pre_cmd{62748} = "chkconfig --list | grep \"^chargen\"";
$pre_cmd{62258} = "chkconfig --list | grep \"^sendmail\"";
$pre_cmd{62571} = "chkconfig --list | grep \"^echo\"";
$pre_cmd{62699} = "chkconfig --list | grep \"^discard\"";
$pre_cmd{62890} = "chkconfig --list | grep \"^nfs\"";
$pre_cmd{62567} = "chkconfig --list | grep \"^nfslock\"";
$pre_cmd{62198} = "chkconfig --list | grep \"^ntalk\"";
$pre_cmd{62106} = "chkconfig --list | grep \"^lpd\"";
$pre_cmd{62977} = "chkconfig --list | grep \"^ypbind\"";
...
$pre_cmd{62994} = "chkconfig --list | grep \"^ident\"";
$pre_cmd{62795} = "chkconfig --list | grep \"^bootps\"";
...
if [ -n \"\$ret\" ];then echo \"red:valid\";else echo \"red-suse:no value\";fi";
$pre_cmd{62162} = "chkconfig --list | grep \"^kshell\"";
$pre_cmd{5243} = "cat /etc/shadow | awk 'BEGIN{FS=\":\";ORS=\",\"}{if(\$2==\"\")print \$1};' | more";
$pre_cmd{62196} = "chkconfig --list | grep \"^time\"";
$pre_cmd{62300} = "chkconfig --list | grep \"^klogin\"";
$pre_cmd{61686} = "if [ -s /etc/pam.d/sshd ];then ret1=`cat /etc/pam.d/sshd  | grep -v \"^[[:space:]]*#\" | grep \"auth[[:space:]]*required[[:space:]]*pam_tally.so[[:space:]]*deny=5[[:space:]]*unlock_time=[0-9]*\"`;fi
if [ -s /etc/pam.d/sshd ];then ret2=`cat /etc/pam.d/sshd  | grep -v \"^[[:space:]]*#\" | grep \"account[[:space:]]*required[[:space:]]*pam_tally.so\"`;fi
...
$pre_cmd{38719} = "if [ -s /etc/pam.d/system-auth ];then red_ret=`cat /etc/pam.d/system-auth | grep -v \"^[[:space:]]*#\" | grep \"auth[[:space:]]*required[[:space:]]*pam_tally.so[[:space:]]*deny=5[[:space:]]*unlock_time=[0-9]*\"`;fi
...
if [ \"\$count\" -ge 1 ];then status=`ps aux | grep -E 'ntp |ntpd ' | grep -cv \"grep -E ntp |ntpd \"`;if [ \"\$status\" -ge 1 ];then echo \"ntp:start\";else echo \"ntp:stop\";fi;else \"ntp:need install ntp service\";fi;
unset count status;";
...
$pre_cmd{11180} = "if [ -f /etc/xinetd.conf ];then ls -al /etc/xinetd.conf | echo \$(echo \"obase=8;ibase=2;\$(awk '{if(/^[d-]/ && NR==1 || NR==2){print substr(\$1, 2)}}'| sed 's/-/0/g'| sed 's/r/1/g'| sed 's/w/1/g'| sed 's/x/1/g')\"|bc);elif [ -f /etc/inetd.conf ];then ls -al /etc/inetd.conf | echo \$(echo \"obase=8;ibase=2;\$(awk '{if(/^[d-]/ && NR==1 || NR==2){print substr(\$1, 2)}}'| sed 's/-/0/g'| sed 's/r/1/g'| sed 's/w/1/g'| sed 's/x/1/g')\"|bc);fi";
$pre_cmd{61438} = "ls -al /etc/";
$pre_cmd{13945} = "cat \"/etc/ssh/sshd_config\"";
$pre_cmd{13192} = "cat /etc/pam.d/login 2>/dev/null | grep -v \"^[[:space:]]*#\" | grep \"^[[:space:]]*auth\"";
...
$appendix_cmd{10} = "df -m 2>/dev/null";$appendix_cmd{5} = "cat  /etc/securetty 2>/dev/null | head -300";$appendix_cmd{4} = "if [ -f /etc/shadow ];then lsattr /etc/shadow 2>/dev/null;fi;
if [ -f /etc/gshadow ];then lsattr /etc/group 2>/dev/null;fi;
if [ -f /etc/passwd ];then lsattr /etc/passwd 2>/dev/null;fi";$appendix_cmd{17} = "cat /etc/vsftpd/chroot_list 2>/dev/null | grep \"^[[:space:]]*[^#]\" | head -300";$appendix_cmd{20} = "cat /etc/ftpaccess 2>/dev/null | grep -v \"^[[:space:]]*#\" | head -300";$appendix_cmd{22} = "version=`lsb_release -a 2>/dev/null | grep \"Description\" | awk -F: '{print \$2}'`;if [ -n \"\$version\" ];then echo \$version;else if [ -z \"\$version\" ]; then echo \"\";else cat /etc/SuSE-release | grep -v \"VERSION\" | grep -v \"PATCHLEVEL\";fi;fi";$appendix_cmd{21} = "rpm -qa | head -100";$appendix_cmd{6} = "chkconfig --list | head -50";$appendix_cmd{9} = "if [ -f /etc/syslog.conf ];then cat /etc/syslog.conf | grep -v \"^[[:space:]]*#\" | head -300;elif [ -f /etc/syslog-ng/syslog-ng.conf ];then cat /etc/syslog-ng/syslog-ng.conf | grep -v \"^[[:space:]]*#\"  | head -300;elif [ -f /etc/rsyslog.conf ];then cat /etc/rsyslog.conf | grep -v \"^[[:space:]]*#\"  | head -300;fi";$appendix_cmd{2} = "cat  /etc/group 2>/dev/null | head -300";$appendix_cmd{1} = "cat /etc/passwd 2>/dev/null | head -300";$appendix_cmd{15} = "if [ -z \"$para{Linux_su_password}\" ];then ps -ef | head -300;else ps -ef |  grep -v \"$para{Linux_su_password}\" | head -300;fi";$appendix_cmd{0} = "uname -a 2>/dev/null";$appendix_cmd{8} = "last -100 2>/dev/null";$appendix_cmd{12} = "(head -20 /var/log/syslog;head -20 /var/log/messages) 2>/dev/null";$appendix_cmd{3} = "cat  /etc/shadow 2>/dev/null | head -300";$appendix_cmd{7} = "ifconfig -a 2>/dev/null";$appendix_cmd{11} = "lastb -100 2>/dev/null";$appendix_cmd{14} = "netstat -anp 2>/dev/null | head -300";$appendix_cmd{19} = "cat /etc/ftpaccess 2>/dev/null | grep \"^[[:space:]]*banner[[:space:]]*\\/.*\" | awk '{print \$2}' | while read user; do cat \$user;done | grep -v \"^[[:space:]]*#\" | head -300";$appendix_cmd{16} = "cat /etc/vsftpd/vsftpd.conf 2>/dev/null | grep \"^[[:space:]]*ftpd_banner\" | head -300";
...
my %os_info = get_os_info();
 ...
 $xml_string .= '<result uuid= "'.'66c221be-6ab2-ef53-1589-fe16877914f4'.'" ip="'.$ipaddr.'" template_time= "2015-09-10 08:16:22'.'">'."\n";
 $xml_string .= "\t".'<initcmd>'."\n";
 ...
 $xml_string .= "\t"."</security>"."\n";
 $xml_string .= "</result>"."\n";
 $xmlfile = $ipaddr."_"."66c221be-6ab2-ef53-1589-fe16877914f4"."_chk.xml";
 print $xmlfile."\n";
